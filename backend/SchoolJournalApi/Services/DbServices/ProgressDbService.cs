...
namespace SchoolJournalApi.Services.DbServices
{
    public class ProgressDbService : DbService, IProgressDbService
    {
        ...
        public async Task<Progress?> FindProgressAsync(int progressId) 
        {
            try 
            {
                return await _db.Progresses.Include(p => p.Lesson).FirstOrDefaultAsync(p => p.Id == progressId);
            }
        ...
        public IQueryable<Progress> GetProgressHistory(int userId, int lessonId) 
        {
            return _db.Progresses.Where(p => p.UserId == userId && p.LessonId == lessonId
                && p.IsUpdated == true);
        }
        ...
        public IQueryable<Progress> GetProgressesForStudentStatistic(int studentId, int journalId) 
        {
            return _db.Progresses.AsNoTracking().Where(p => p.UserId == studentId && p.Lesson.JournalId == journalId
                && p.MarkId != null && p.IsUpdated == false)
